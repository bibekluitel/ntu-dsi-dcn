...
//
	...
	char filename [] = "statistics/fattree/Fat-tree.xml";// filename for Flow Monitor xml output file
...
//
	...
	for (i=0;i<total_host;i++){
		app[i].Start (Seconds (0.0));
  		app[i].Stop (Seconds (19.0));
	}
  	Ipv4GlobalRoutingHelper::PopulateRoutingTables ();


//calculate traceffile
			AsciiTraceHelper ascii;
			p2p.EnableAsciiAll(ascii.CreateFileStream("statistics/fattree/p2p.tr"));  //p2p is  a pointToPointHelper
			csma.EnableAsciiAll(ascii.CreateFileStream("statistics/fattree/csma.tr"));  // csma is  a csmaHelper

//enable pcap
			p2p.EnablePcapAll("statistics/fattree/pcap/fat-treep2p");	// p2p is  a pointToPointHelper
			csma.EnablePcapAll("statistics/fattree/pcap/fat-treecsma");     // csma is  a csmaHelper
...
//calculating the manual through put from each flow
