...
//
	...
	char filename [] = "statistics/bcube/BCube.xml";	// filename for Flow Monitor xml output file
...
//	
	...
	for (i=0;i<num_sw;i++){
		...
		temp = j;
		for (j=temp;j<temp+n; j++){//0tp 4 next loop 4to 8 8-12
...
//calculate traceffile
			...
			csma.EnableAsciiAll(ascii.CreateFileStream("statistics/bcube/bcubecsma.tr"));  // csma is  a csmaHelper

//enable pcap
			csma.EnablePcapAll("statistics/bcube/pcap/bcubecsma");     // csma is  a csmaHelper
...
//calculating manual throughtput for each flow
